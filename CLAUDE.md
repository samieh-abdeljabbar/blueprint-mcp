...
Blueprint MCP is a Model Context Protocol server that maintains a living architectural map of any software project. It tracks components (services, databases, APIs, routes, functions, etc.), their connections, and build status via a local SQLite database. Currently exposes **39 MCP tools** with **353 tests**.
